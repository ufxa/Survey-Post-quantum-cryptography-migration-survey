...
Proposed heuristic decision-support rubric for PQC migration prioritization.

IMPORTANT FRAMING: MRC is a *proposed conceptual framework*, not a validated
quantitative instrument. Weights are author-assigned heuristic values; they
have not been derived from a documented expert-elicitation process. The AHP
matrix in the companion paper requires correction before claiming CR=0.04
or weights of 0.45/0.35/0.20 via AHP. See AUDIT_LOG.md for details.

Usage:
    python mrc_scorer.py --domain TLS
    python mrc_scorer.py --all
    python mrc_scorer.py --all --json
    python mrc_scorer.py --all --reference-year 2027

REMOVED FLAGS (were documented but never implemented):
    --interactive  [removed — was never implemented]
    --batch        [removed — was never implemented]

Reference:
    "A Comprehensive Survey on Post-Quantum Cryptography Migration"
    IEEE Communications Surveys & Tutorials, 2026 (under review)
"""
...
# ─── Weights ─────────────────────────────────────────────────────────────────
# AUDIT NOTE: These are author-assigned heuristic weights. The companion AHP
# matrix (E=1,3,5; F=1/3,1,3; D=1/5,1/3,1) actually produces w=[0.633,0.261,0.106]
# with CR=0.033 — NOT 0.45/0.35/0.20. The paper's AHP claim is incorrect.
# Until a properly documented expert-elicitation is conducted, these weights
# should be described as "author-assigned" or "heuristic". See mrc_weights_audit.py
# for the corrected AHP computation.
W_EXPOSURE    = 0.45   # Cryptographic exposure (heuristic weight)
W_FEASIBILITY = 0.35   # Upgrade feasibility (inverse: 1 - F; heuristic weight)
W_DEADLINE    = 0.20   # Regulatory deadline proximity (inverse: 1 - D; heuristic weight)

assert abs(W_EXPOSURE + W_FEASIBILITY + W_DEADLINE - 1.0) < 1e-9, "Weights must sum to 1.0"

DEADLINE_WINDOW_YEARS = 10  # Normalization window for deadline score
DEFAULT_REFERENCE_YEAR = 2026  # Updated via --reference-year CLI flag
...
class RegulatoryContext:
    ...
    def get_earliest_deadline(self, reference_year: int = DEFAULT_REFERENCE_YEAR) -> int:
...
def compute_deadline_score(regulatory: Optional[RegulatoryContext],
    ...
    AUDIT NOTE: When regulatory context is absent, the original code returned 0.5
    (moderate urgency), which is a favorable default. For unknown regulatory
    context, 0.5 is retained as a deliberate midpoint assumption — callers should
    provide explicit context rather than relying on this default.

    When a deadline has already passed (years_remaining < 0), returns 0.0
    (maximum urgency / past deadline) rather than a negative value.
    """
    if regulatory is None:
        return 0.5  # EXPLICIT ASSUMPTION: unknown regulatory context = moderate urgency
    earliest = regulatory.get_earliest_deadline(reference_year)
    years_remaining = earliest - reference_year
    if years_remaining < 0:
        # Deadline already passed — maximum urgency
        return 0.0
...
def build_domain_profiles(reference_year: int = DEFAULT_REFERENCE_YEAR) -> List[SystemProfile]:
...
def main():
    parser = argparse.ArgumentParser(
        description='MRC Heuristic Decision-Support Rubric for PQC Migration Prioritization')
    ...
    parser.add_argument('--reference-year', type=int, default=DEFAULT_REFERENCE_YEAR,
                        help=f'Reference year for deadline computation (default: {DEFAULT_REFERENCE_YEAR})')
    args = parser.parse_args()

    profiles = build_domain_profiles(reference_year=args.reference_year)
