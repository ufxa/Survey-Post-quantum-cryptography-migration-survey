...
\thanks{Manuscript submitted July 15, 2026. This research was supported by FAPESPA, PRODEPA, RNP, and INCT iAmazonia (institutional and financial details disclosed upon acceptance per double-blind policy).}%
...
a structured narrative review encompassing 72 peer-reviewed
...
\begin{enumerate}
  ...
  \item \textbf{Structured narrative survey} synthesizing 72 peer-reviewed
...
The assembled corpus consists of \textbf{72 peer-reviewed publications} and
...
evidence base refer to the 72-paper peer-reviewed corpus; regulatory and
...
\begin{figure}[t]
  ...
  was de-duplicated (9 redundant entries removed) and split into 72
...
\begin{figure}[t]
  ...
  by sub-theme. Stacked bars show observed counts from the 72-paper corpus.
...
for all 111 surveyed sources (72 peer-reviewed + 39 normative).
...
\textbf{Sensitivity analysis}: Table~\ref{tab:mrc_sensitivity} shows tier
assignments for the six reference profiles under three weight perturbations.
No tier assignment changes under perturbations of $\pm 0.10$ on any single
weight (with the other two adjusted proportionally), confirming robustness
for the reference profiles constructed in this survey.

\begin{table}[t]
\caption{MRC Sensitivity Analysis: Tier Assignments Under Weight Perturbations}
\label{tab:mrc_sensitivity}
\centering
\footnotesize
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}L{2.0cm}C{1.0cm}C{1.0cm}C{1.0cm}C{1.0cm}@{}}
\toprule
\textbf{Domain} & \textbf{Base} & \multicolumn{3}{c}{\textbf{Tier under perturbed weights}} \\
\cmidrule(l){3-5}
& \textbf{(0.45,0.35,0.20)} & $w_E{+}0.1$ & $w_F{+}0.1$ & $w_D{+}0.1$ \\
\midrule
TLS/Web    & T2 & T2 & T2 & T2 \\
PKI        & T1 & T1 & T1 & T1 \\
IoT        & T2 & T2 & T2 & T2 \\
...
\bottomrule
\multicolumn{5}{@{}l}{\scriptsize Perturbed weights renormalized to sum to 1.0.}
\end{tabular}
\end{table}
...
72 peer-reviewed publications and 39 normative documents.
